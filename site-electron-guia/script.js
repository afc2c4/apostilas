...
function parseTable(lines, startIndex) {
  ...
  if (!headerLine.includes('|') || !/^\s*\|?\s*[-:|\s]+\|?\s*$/.test(separatorLine)) {
    return null;
  }
...
function setupActiveSection(headers) {
...
async function init() {
  ...
  try {
    ...
    setupSearch();
    setupActiveSection(headers);
  } catch (error) {
    article.innerHTML = `<p>Erro ao carregar o conteúdo: ${escapeHtml(error.message)}</p>`;
  }
}
